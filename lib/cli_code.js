...
const debug = require('debug')('mhio:test-cli:cli_code')
const { Exception } = require('@mhio/exception')


class CliCodeException extends Exception {}
...
class CliCode {
  ...
  replaceStderr(){
    ...
    process.stderr.write = (chunk, encoding, fd) => {
      ...
      // The `debug` module output from anything can be logged norally
      // otherwise stderr tests start failing
      if (
        (
          /^\s\s\u001b\[\d\d;1m\w/.exec(chunk)
          || /^\s\s\u001b\[\d\d;1m\w/.exec(chunk)
        )
        && /\d(?:ms|s|m|h)\u001b\[0m\n$/.exec(chunk)
      ){
  ...
  run () {
    return new Promise((resolve, reject) => {
      if (!this.function) throw new CliCodeException('CliCode requires a function to run')

      this.setup(resolve, reject)
      debug('returned from setup')

      if (this.stdout && !this.process_stdout_write)
        reject(new CliCodeException('Something wrong with stdout setup'))

      if (this.stderr && !this.process_stderr_write)
        reject(new CliCodeException('Something wrong with stderr setup'))

      if (!this.process_argv)
        reject(new CliCodeException('Something wrong with the argv replacement setup'))

      if (!this.process_exit)
        reject(new CliCodeException('Something wrong with the exit replacement setup'))
